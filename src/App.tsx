...
export default function App() {
  ...
  const portfolio = [
    { logo: '/logo-kb-consulting.png', logoAlt: 'KB Consulting', tag: 'Consultoría & Estrategia', desc: 'Consultoría especializada en inversiones, trading y estrategia digital. Asesoramiento personalizado para emprendedores y empresas que quieren incursionar en el mundo crypto y Web3.', chips: ['Consultoría', 'Trading', 'Web3', 'Estrategia'], link: 'https://linktr.ee/kevinbarquerocrypto' },
    { logo: '/logo-bbr-tek.png', logoAlt: 'BBR Tek', tag: 'Tecnología & Desarrollo', desc: 'Soluciones tecnológicas innovadoras para empresas. Desarrollo de software, integraciones blockchain y transformación digital para negocios en Latinoamérica.', chips: ['Tech', 'Blockchain', 'Software', 'Latam'], link: 'https://linktr.ee/kevinbarquerocrypto' },
    { logo: '/logo-bell-n-desk.png', logoAlt: 'Bell n Desk', tag: 'Plataforma Digital', desc: 'Plataforma de gestión y comunicación digital. Conectando equipos y optimizando procesos empresariales con tecnología de vanguardia.', chips: ['SaaS', 'Gestión', 'Comunicación', 'Digital'], link: 'https://linktr.ee/kevinbarquerocrypto' },
    { logo: '/logo-kf-tech.png', logoAlt: 'KF Tech', tag: 'Innovación Tecnológica', desc: 'Soluciones de tecnología de punta para el ecosistema financiero y blockchain. Infraestructura robusta y segura para proyectos DeFi y Web3 en la región.', chips: ['Fintech', 'DeFi', 'Infraestructura', 'Web3'], link: 'https://linktr.ee/kevinbarquerocrypto' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-[#1d1d1f] font-sans">
      ...
      <section id="hero" className="relative min-h-screen overflow-hidden">
        {/* Background */}
        <div className="absolute inset-0 bg-gradient-to-br from-gray-200 via-gray-50 to-white pointer-events-none" />
        <div className="absolute inset-0 bg-cover bg-center opacity-[0.04]" style={{ backgroundImage: 'url(/hero-bg.jpg)' }} />

        {/* Photo — absolute right side, bottom-anchored */}
        <div className="absolute right-20 bottom-0 h-full w-1/2 flex items-end justify-end pointer-events-none select-none">
          <img
            src="/kevin-hero.png"
            alt="Kevin Barquero"
            className="h-[80%] w-auto object-contain object-bottom"
            style={{ maskImage: 'linear-gradient(to bottom, transparent 0%, black 20%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, black 8%)' }}
          />
        </div>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 lg:px-8 min-h-screen flex flex-col justify-center pt-24 pb-20">
          <div className="max-w-xl">
            <FadeSection>
              ...
              <h1 className="text-[60px] sm:text-[76px] lg:text-[92px] font-bold leading-[1.02] tracking-tight text-[#1d1d1f] mb-6">
                Kevin<br /><span className="text-[#6e6e73]">Barquero</span>
              </h1>
              <p className="text-[18px] sm:text-[20px] text-[#6e6e73] leading-relaxed mb-10">
                Especialista en Web4, Blockchain y Trading. Ayudando a personas a alcanzar su libertad financiera desde Costa Rica 🇨🇷
              </p>
      ...
      <section className="border-y border-gray-100 bg-[#f5f5f7] py-10">
        <FadeSection>
          <div className="max-w-4xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {stats.map((s, i) => (
              <div key={i}>
                <p className="text-[36px] font-bold tracking-tight text-[#1d1d1f]">{s.value}</p>
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <FadeSection>
            <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
              <div>
                <p className="text-[12px] font-semibold tracking-widest uppercase text-[#6e6e73] mb-4 flex items-center gap-2"><Award size={14} /> Sobre Mí</p>
                <h2 className="text-[40px] sm:text-[48px] font-bold tracking-tight leading-[1.1] mb-7 text-[#1d1d1f]">
                  Inversionista &<br />Emprendedor Digital.
                </h2>
                <div className="space-y-4 text-[16px] text-[#6e6e73] leading-relaxed">
              ...
              <div className="relative flex flex-col items-center">
                ...
                />
                <div className="relative z-10 mt-4 bg-white border border-gray-100 rounded-2xl px-6 py-4 flex items-center gap-4 shadow-sm w-full max-w-[360px]">
      ...
      {/* SERVICES */}
      <section id="consulting" className="py-28 lg:py-36 bg-[#f5f5f7]">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <FadeSection>
            <div className="text-center mb-16">
              <p className="text-[12px] font-semibold tracking-widest uppercase text-[#6e6e73] mb-4">Consultoría</p>
              <h2 className="text-[40px] sm:text-[48px] font-bold tracking-tight leading-[1.1] text-[#1d1d1f] mb-4">Servicios.</h2>
              <p className="text-[18px] text-[#6e6e73] max-w-xl mx-auto">Navega el mundo de las criptomonedas, blockchain e inversiones digitales con guía experta.</p>
            </div>
          </FadeSection>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {services.map((s, i) => (
              <FadeSection key={i}>
                <div className="bg-white rounded-2xl p-7 hover:shadow-md transition-shadow h-full">
                  <div className="w-11 h-11 rounded-xl bg-[#f5f5f7] flex items-center justify-center text-[#1d1d1f] mb-5">{s.icon}</div>
                  <h3 className="font-semibold text-[17px] text-[#1d1d1f] mb-2">{s.title}</h3>
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <FadeSection>
            <div className="text-center mb-16">
              ...
              <h2 className="text-[40px] sm:text-[48px] font-bold tracking-tight leading-[1.1] text-[#1d1d1f] mb-4">Empresas & Proyectos.</h2>
              <p className="text-[18px] text-[#6e6e73] max-w-xl mx-auto">Proyectos innovadores en el ecosistema blockchain y fintech de Latinoamérica.</p>
            </div>
          </FadeSection>
          <div className="grid lg:grid-cols-2 gap-5">
            {portfolio.map((p, i) => (
              <FadeSection key={i}>
                <div className="bg-[#f5f5f7] rounded-2xl p-8 hover:bg-[#ebebed] transition-colors h-full flex flex-col">
                  <p className="text-[11px] font-semibold tracking-widest uppercase text-[#6e6e73] mb-4">{p.tag}</p>
                  <a href={p.link} target="_blank" rel="noopener noreferrer" className="mb-5 inline-block group">
                    <img
                      src={p.logo}
                      alt={p.logoAlt}
                      className="h-12 w-auto object-contain object-left transition-opacity group-hover:opacity-70"
                    />
                  </a>
                  <p className="text-[15px] text-[#6e6e73] leading-relaxed mb-6 flex-1">{p.desc}</p>
      ...
      {/* CONTACT */}
      <section id="contact" className="py-28 lg:py-36 bg-[#1d1d1f] text-white">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <FadeSection>
            <div className="text-center mb-16">
              <p className="text-[12px] font-semibold tracking-widest uppercase text-[#86868b] mb-4">Contacto</p>
              <h2 className="text-[40px] sm:text-[56px] font-bold tracking-tight leading-[1.05] mb-4">Trabajemos Juntos.</h2>
              <p className="text-[18px] text-[#86868b] max-w-xl mx-auto">¿Tenés un proyecto en mente o querés colaborar? Estoy aquí.</p>
            </div>
          </FadeSection>
          <div className="grid lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="space-y-3">
                {socials.map((s, i) => (
                  <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-4 p-5 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors border border-white/10 group">
                    <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-white">{s.icon}</div>
                    <div>
                      <p className="font-medium text-[15px]">{s.label}</p>
                      <p className="text-[13px] text-[#86868b]">{s.handle}</p>
                    </div>
                    <ArrowRight size={16} className="ml-auto text-[#86868b] group-hover:text-white transition-colors" />
                  </a>
                ))}
              </div>
            </FadeSection>
            <FadeSection>
              <div className="bg-white/5 border border-white/10 rounded-3xl p-10 text-center">
                ...
                <h3 className="text-[24px] font-bold mb-3">¿Listo para Empezar?</h3>
                <p className="text-[15px] text-[#86868b] leading-relaxed mb-8">Ya sea consultoría, colaboración o simplemente charlar sobre Web3 y cripto — no dudes en escribirme.</p>
